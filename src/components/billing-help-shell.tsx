...
const billingSidebarIndexMap = Object.fromEntries(
  billingSidebarEntries.map((entry) => [entry.slug, entry.index]),
);

type BillingHelpShellProps = {
  article: HelpArticle;
};
...
function BillingSidebar({ currentSlug }: { currentSlug: string }) {
  return (
    <nav className="space-y-4">
      <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#b9b9b9]">Credits & Billing</p>
      <ol className="space-y-1.5">
        {billingSidebarEntries.map((entry) => {
            ...
            <li key={entry.slug}>
              <Link
                ...
                className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${
                  isActive ? "bg-[#fff4ec] text-[#e98532]" : "text-[#787878] hover:bg-[#f6f6f6] hover:text-[#343434]"
                }`}
                aria-current={isActive ? "page" : undefined}
              >
                {isActive ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-[#ed8a3a]" aria-hidden /> : null}
                <span className="w-6 shrink-0 text-[11px] font-medium tracking-[0.2em] text-[#bfbfbf]">{entry.index}</span>
...
function BillingFaqList({ items }: { items: BillingFaqItem[] }) {
  return (
    <div className="space-y-4">
      {items.map((item) => (
        <details
          key={item.question}
          className="group rounded-[1.2rem] border border-[rgba(15,23,42,0.08)] bg-[linear-gradient(180deg,#ffffff_0%,#fbfaf8_100%)] px-5 py-4 shadow-[0_14px_28px_rgba(15,23,42,0.05)]"
        >
          <summary className="cursor-pointer list-none text-base font-medium text-[#1f1f1f] marker:hidden">
            <span className="inline-flex items-center gap-3">
              <span className="text-[#ed8a3a] transition group-open:rotate-45">＋</span>
              <span>{item.question}</span>
            </span>
          </summary>
          <p className="mt-4 pl-7 text-[15px] leading-7 text-[#555]">{item.answer}</p>
        </details>
      ))}
    </div>
  );
...
function BillingArticleBody({ article }: { article: HelpArticle }) {
  const paragraphs = article.heroParagraphs?.length ? article.heroParagraphs : [article.description];
  const displayIndex = billingSidebarIndexMap[article.slug] ?? article.index;
  ...
  return (
    <article className="min-w-0">
      <div className="border-b border-black/8 pb-6">
        ...
        <h2 className="mt-3 text-[2rem] font-semibold tracking-[-0.035em] text-[#222] sm:text-[2.25rem]">{article.title}</h2>
      </div>
        ...
        {article.sections.map((section) => {
          ...
          return (
            <section key={section.id} id={section.id} className="scroll-mt-24">
              ...
              <div className={isFaq ? "space-y-4" : "mt-4 space-y-4"}>
                {isFaq ? (
                  <BillingFaqList items={billingFaqItems} />
                ) : (
                  sectionBlocks.map((block, blockIndex) =>
                    block.type === "paragraph" ? (
                      ...
                      <ul key={`${section.id}-list-${blockIndex}`} className="space-y-3 text-[15px] leading-7 text-[#4f4f4f]">
      ...
      {article.nextHref && article.nextLabel && !article.relatedLinks?.length ? (
...
export function BillingHelpShell({ article }: BillingHelpShellProps) {
  return (
    <main className="min-h-screen bg-[#fcfcfb] text-[#1f1f1f]">
      <div className="mx-auto max-w-[1440px] px-6 py-6 sm:px-8 lg:px-10">
        ...
        <div className="grid gap-10 pt-8 xl:grid-cols-[260px_minmax(0,720px)_200px] xl:gap-12">
          <BillingSidebar currentSlug={article.slug} />
          <BillingArticleBody article={article} />
...
export function getBillingHelpArticle(slug: string) {
  if (slug !== "billing" && slug !== "faq") {
    return undefined;
  }
