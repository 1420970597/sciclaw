...
function FeatureNetwork({ activeIndex, onSelect }: { activeIndex: number; onSelect: (index: number) => void }) {
  const nodes = useMemo(
    () => [
    {
      ...
      labelClass: "max-w-[96px] text-left text-[#314d61] drop-shadow-[0_1px_0_rgba(255,255,255,0.98)]",
      activeLabelClass: "max-w-[96px] text-left text-[#5f381d] drop-shadow-[0_2px_3px_rgba(240,142,79,0.014)]",
      labelPositionClass: "absolute left-[-3.08rem] top-[0.34rem] sm:left-[-3.18rem]",
    },
    {
      id: "data-mining",
      label: "Data Mining",
      ...
      activeRingClass: "h-[5.02rem] w-[5.02rem] border-[#cf884f] bg-[linear-gradient(180deg,#fffdf9_0%,#ffe7d3_100%)] text-[#9f5628] shadow-[0_0_0_2px_rgba(240,142,79,0.035),0_13px_24px_rgba(232,124,55,0.082)]",
      wrapperClass: "items-center",
      labelClass: "max-w-[96px] text-center text-[#314d61] drop-shadow-[0_1px_0_rgba(255,255,255,0.98)]",
      activeLabelClass: "max-w-[96px] text-center text-[#5f381d] drop-shadow-[0_2px_3px_rgba(240,142,79,0.014)]",
      labelPositionClass: "absolute left-1/2 top-[calc(100%+0.5rem)] -translate-x-1/2",
    },
    {
      id: "outcome-present",
      ...
      labelClass: "max-w-[96px] text-left text-[#314d61] drop-shadow-[0_1px_0_rgba(255,255,255,0.98)]",
      activeLabelClass: "max-w-[96px] text-left text-[#5f381d] drop-shadow-[0_2px_3px_rgba(240,142,79,0.014)]",
      labelPositionClass: "absolute left-[3.28rem] top-[0.34rem] sm:left-[3.4rem]",
    },
    ],
    [],
  );
...
function FeaturePreview({
  ...
  return (
    <div className="flex flex-col gap-[1.32rem] lg:grid lg:grid-cols-[minmax(0,1fr)_196px] lg:items-start lg:gap-x-[0.02rem] lg:gap-y-[0.9rem] xl:grid-cols-[minmax(0,1fr)_196px] xl:gap-x-[0.08rem] xl:gap-y-[0.98rem]">
      ...
      <div className="order-3 -mt-[0.14rem] lg:-ml-[0.16rem] lg:order-2 lg:col-start-2 lg:row-start-2 lg:ml-auto lg:mt-[-0.02rem] lg:max-w-[12.35rem] xl:-ml-[0.22rem] xl:mt-[0.06rem]">
            ...
            <div className="rounded-[1.44rem] border border-[#ead7c5] bg-white/95 p-[1.34rem] shadow-[0_8px_20px_rgba(15,23,42,0.032),inset_0_1px_0_rgba(255,255,255,0.78)]">
              <div className="space-y-[1.54rem]">
                ...
                <div
                  ...
                  className={`mt-[2.54rem] grid ${chartGridClass} gap-[3.18rem] px-[1.1rem] pt-[2.38rem] sm:gap-[3.32rem]`}
                >
                  {visibleBars.map((height, index) => (
                    <div key={`${activeFeature.id}-${index}`} className="flex min-w-[0] flex-1 flex-col items-center gap-[0.94rem] text-center sm:gap-[1rem]">
                      ...
                      <span className="flex min-h-[4.36rem] w-full min-w-0 items-start justify-center px-[0.72rem] pb-[0.04rem] text-center text-[11.2px] font-medium uppercase leading-[1.08] tracking-[0.028em] text-[#6b8191] sm:min-h-[4.52rem] sm:text-[11.36px] sm:tracking-[0.034em]">
                        <span className="block max-w-[6.48rem] text-balance break-words">{visibleChartLabels[index] ?? `Q${index + 1}`}</span>
                      </span>
                    </div>
                  ))}
                </div>
...
function FeatureRotator({
  ...
  return (
    ...
    >
      <div className="overflow-visible rounded-[2.05rem] bg-[radial-gradient(circle_at_center,_rgba(255,255,255,0.46),_rgba(255,255,255,0)_66%)] pl-[0.18rem] pr-0 py-1 sm:px-1 lg:-mt-[2.18rem] lg:-mr-[0.02rem] lg:px-0 lg:pl-[0.2rem] xl:-mt-[2.42rem] xl:-mr-[0.08rem] xl:px-1 xl:pl-[0.24rem]">
...
function AuthCard({
  ...
  return (
    <div
      ...
      className="mx-auto w-full max-w-[186px] rounded-[1.08rem] border border-[rgba(244,247,250,0.97)] bg-[linear-gradient(180deg,rgba(255,255,255,0.994)_0%,rgba(251,247,242,0.94)_100%)] px-[0.24rem] py-[0.3rem] shadow-[0_2px_5px_rgba(15,23,42,0.018),0_1px_2px_rgba(241,180,135,0.009)] backdrop-blur-[0.8px] sm:px-[0.28rem] sm:py-[0.34rem]"
    >
