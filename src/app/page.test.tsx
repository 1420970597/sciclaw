...
describe("Home landing page", () => {
  ...
  it("renders the public-inspired landing structure", () => {
    ...
    expect(featureRotator).toHaveClass("lg:gap-[0.06rem]");
    expect(featureRotator).toHaveClass("xl:grid-cols-[minmax(0,342px)_minmax(0,1fr)]");
    expect(featureRotator).toHaveClass("xl:gap-[0.1rem]");
    ...
    expect(networkShellFrame).toHaveClass("lg:-mt-[0.28rem]");
    expect(networkShellFrame).toHaveClass("lg:-mr-[0.08rem]");
    expect(networkShellFrame).toHaveClass("xl:-mt-[0.38rem]");
    expect(networkShellFrame).toHaveClass("xl:-mr-[0.12rem]");
    ...
    expect(utilityRow).toHaveClass("sm:gap-[0.62rem]");
    expect(utilityRow).toHaveClass("sm:pr-[1.22rem]");
    ...
    expect(bestCasesSection).toHaveClass("lg:mt-[5.82rem]");
    expect(bestCasesSection).toHaveClass("xl:mt-[6.2rem]");
    expect(bestCasesSection.firstElementChild).toHaveClass("mb-[2.3rem]");
    ...
    expect(featurePreview).not.toHaveClass("lg:grid");
    expect(featurePreview).not.toHaveClass("lg:grid-cols-[minmax(0,1fr)_218px]");
    expect(featurePreview).not.toHaveClass("lg:items-start");
    expect(featurePreview).not.toHaveClass("lg:gap-x-[0.02rem]");
    expect(featurePreview).not.toHaveClass("lg:gap-y-[0.64rem]");
    expect(featurePreview).not.toHaveClass("xl:grid-cols-[minmax(0,1fr)_218px]");
    expect(featurePreview).not.toHaveClass("xl:gap-x-[0.04rem]");
    expect(featurePreview).not.toHaveClass("xl:gap-y-[0.72rem]");
    const previewHeadingBlock = previewCard.parentElement?.firstElementChild;
    expect(previewHeadingBlock).not.toBeNull();
    expect(previewHeadingBlock).toHaveClass("order-1");
    expect(previewHeadingBlock).not.toHaveClass("lg:col-span-2");
    expect(previewCard).toHaveClass("order-2");
    expect(previewCard).not.toHaveClass("lg:col-start-1");
    ...
    expect(authColumn).toHaveClass("max-w-[168px]");
    expect(authColumn).toHaveClass("sm:max-w-[180px]");
    expect(authColumn).toHaveClass("lg:ml-auto");
    expect(authColumn).toHaveClass("lg:mr-[0.01rem]");
    expect(authColumn).toHaveClass("lg:-mt-[14.72rem]");
    expect(authColumn).toHaveClass("lg:max-w-[216px]");
    expect(authColumn).toHaveClass("xl:mr-[0.12rem]");
    expect(authColumn).toHaveClass("xl:-mt-[14.96rem]");
    expect(authColumn).toHaveClass("xl:max-w-[236px]");
    expect(authColumn).not.toHaveClass("-mt-[2rem]");
    expect(authColumn).not.toHaveClass("max-w-[172px]");
    expect(authColumn).not.toHaveClass("sm:max-w-[184px]");
    expect(authColumn).not.toHaveClass("lg:mr-[0.03rem]");
    expect(authColumn).not.toHaveClass("xl:mr-[0.16rem]");
    expect(authColumn).not.toHaveClass("lg:-mt-[15.02rem]");
    expect(authColumn).not.toHaveClass("xl:-mt-[15.28rem]");
    expect(authColumn).not.toHaveClass("lg:max-w-[224px]");
    expect(authColumn).not.toHaveClass("xl:max-w-[244px]");
    expect(authColumn).not.toHaveClass("max-w-[176px]");
    expect(authColumn).not.toHaveClass("sm:max-w-[188px]");
    expect(authColumn).not.toHaveClass("lg:mr-[0.06rem]");
    expect(authColumn).not.toHaveClass("xl:mr-[0.2rem]");
    expect(authColumn).not.toHaveClass("lg:-mt-[15.18rem]");
    expect(authColumn).not.toHaveClass("xl:-mt-[15.46rem]");
    expect(authColumn).not.toHaveClass("lg:max-w-[230px]");
    expect(authColumn).not.toHaveClass("xl:max-w-[250px]");
    ...
    expect(screen.getByTestId("feature-node-data-mining").parentElement?.className).toContain("left-[45.04%]");
    ...
    expect(screen.getByText(/^outcome present$/i)).toHaveClass("top-[0.38rem]");
    ...
    expect(authCard).toHaveClass("max-w-[204px]");
    expect(authCard).toHaveClass("lg:max-w-[452px]");
    expect(authCard).toHaveClass("xl:max-w-[468px]");
    expect(authCard).toHaveClass("rounded-[0.7rem]");
    expect(authCard).toHaveClass("border-[rgba(226,232,239,0.9)]");
    expect(authCard).toHaveClass("bg-[linear-gradient(180deg,rgba(255,255,255,0.982)_0%,rgba(249,245,239,0.79)_100%)]");
    expect(authCard).toHaveClass("shadow-[0_8px_18px_rgba(15,23,42,0.032),0_3px_6px_rgba(241,180,135,0.034)]");
    expect(authCard).toHaveClass("backdrop-blur-[3px]");
    expect(authCard).toHaveClass("mx-auto");
    expect(authCard.firstElementChild).toHaveClass("max-w-[244px]");
    expect(authCard.firstElementChild).toHaveClass("lg:max-w-[472px]");
    expect(authCard.firstElementChild).toHaveClass("xl:max-w-[488px]");
    expect(authCard.firstElementChild?.firstElementChild).toHaveClass("rounded-[0.78rem]");
    expect(authCard).not.toHaveClass("max-w-[208px]");
    expect(authCard).not.toHaveClass("lg:max-w-[460px]");
    expect(authCard).not.toHaveClass("xl:max-w-[476px]");
    expect(previewCard).toHaveClass("rounded-[2.08rem]");
    expect(previewCard).toHaveClass("border-[rgba(176,190,204,0.98)]");
    expect(previewCard).toHaveClass("bg-[linear-gradient(180deg,#f9f2e8_0%,#d5e0ea_100%)]");
    expect(previewCard).toHaveClass("shadow-[0_30px_60px_rgba(15,23,42,0.17)]");
    ...
    expect(previewLabelRow).toHaveClass("pt-[1.12rem]");
  ...
  it("keeps the utility triggers subtle, preserves the settings menu contract, and improves hero cluster/label readability after the cohesion pass", () => {
    ...
    expect(literatureNode.parentElement?.className).toContain("left-[45.36%]");
    expect(dataMiningNode.parentElement?.className).toContain("left-[45.04%]");
    expect(outcomeNode.parentElement?.className).toContain("right-[14.2%]");
    expect(literatureNodeWrapper?.parentElement?.querySelector("span[class*='w-[12.9%]']")).not.toBeNull();
    expect(dataMiningNodeWrapper?.parentElement?.querySelector("span[class*='w-[13.62%]']")).not.toBeNull();
    ...
    expect(dataMiningNode.firstElementChild).toHaveClass("h-[4.58rem]");
    expect(outcomeNode.firstElementChild).toHaveClass("h-[4.58rem]");
    ...
    chartLabels.forEach((label) => {
      ...
      expect(labelWrapper).not.toBeNull();
      expect(labelWrapper).toHaveClass("min-h-[2.92rem]");
      expect(labelWrapper).toHaveClass("text-[11.1px]");
      expect(labelWrapper).toHaveClass("leading-[1.06]");
      expect(labelWrapper).toHaveClass("tracking-[0.018em]");
      expect(labelWrapper).toHaveClass("text-[#6b8191]");
      expect(label).toHaveClass("max-w-[5.5rem]");
    ...
    expect(chartRow).toHaveClass("gap-[2.18rem]");
    expect(chartRow).toHaveClass("sm:gap-[2.34rem]");
    expect(chartRow).toHaveClass("mt-[1.46rem]");
    expect(chartRow).toHaveClass("pt-[1.12rem]");
    ...
    expect(literatureNodeLabel).toHaveClass("max-w-[128px]");
    expect(literatureNodeLabel).toHaveClass("whitespace-nowrap");
    expect(literatureNodeLabel).toHaveClass("left-[-1.72rem]");
    expect(literatureNodeLabel).toHaveClass("top-[0.34rem]");
    expect(literatureNodeLabel).toHaveClass("sm:left-[-1.82rem]");
    expect(literatureNodeLabel).toHaveClass("text-left");
    expect(literatureNodeLabel).toHaveClass("text-[#5f381d]");
    expect(dataMiningNodeLabel).toHaveClass("max-w-[130px]");
    ...
    expect(dataMiningNodeLabel).toHaveClass("top-[calc(100%+0.02rem)]");
    ...
    expect(outcomeNodeLabel).toHaveClass("top-[0.38rem]");
