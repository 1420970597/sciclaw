...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(within(chatHero).getByTestId("feature-node-literature-analysis").parentElement?.className).toContain("left-[42.2%]");
    expect(within(chatHero).getByTestId("feature-node-data-mining").parentElement?.className).toContain("left-[41.9%]");
    expect(within(chatHero).getByTestId("feature-node-outcome-present").parentElement?.className).toContain("right-[15.3%]");
    ...
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("left-[-2.12rem]");
    ...
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("left-[1.94rem]");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("top-[0.48rem]");
    expect(within(chatHero).getByText(/^outcome present$/i)).not.toHaveClass("-translate-y-1/2");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("sm:left-[2.02rem]");
    ...
    expect(chatHero.querySelector("span[class*='w-[16.4%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[17%]']")).not.toBeNull();
