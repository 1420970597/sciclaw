...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(chatHero).toHaveClass("lg:grid-cols-[minmax(0,1fr)_234px]");
    ...
    expect(within(chatHero).getByTestId("feature-node-literature-analysis").parentElement?.className).toContain("left-[45.15%]");
    expect(within(chatHero).getByTestId("feature-node-data-mining").parentElement?.className).toContain("left-[45.35%]");
    expect(within(chatHero).getByTestId("feature-node-outcome-present").parentElement?.className).toContain("right-[13.45%]");
    ...
    expect(chatHero.querySelector("span[class*='w-[13.9%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[14%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[13.1%]']")).not.toBeNull();
    expect(chatHero).toHaveClass("xl:-mt-[2.28rem]");
    expect(chatHero).toHaveClass("xl:grid-cols-[minmax(0,1.1fr)_234px]");
    expect(chatHero).toHaveClass("xl:gap-[0.16rem]");
    expect(chatAuthCard).toHaveClass("max-w-[234px]");
    expect(chatAuthCard).toHaveClass("bg-[#fdfdff]");
    expect(chatAuthCard).toHaveClass("border-[#f5f7fa]");
    expect(chatAuthCard).toHaveClass("shadow-[0_1px_2px_rgba(15,23,42,0.005),0_1px_2px_rgba(241,180,135,0.002)]");
    expect(chatAuthCard).toHaveClass("lg:mt-[1.78rem]");
    expect(chatAuthCard).toHaveClass("xl:mt-[1.96rem]");
