...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(within(chatHero).getByTestId("feature-node-literature-analysis").parentElement?.className).toContain("left-[44.2%]");
    expect(within(chatHero).getByTestId("feature-node-data-mining").parentElement?.className).toContain("left-[44.05%]");
    ...
    expect(chatHero.querySelector("span[class*='w-[14.4%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[15%]']")).not.toBeNull();
