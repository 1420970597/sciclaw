...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(within(chatHero).getByTestId("feature-node-outcome-present").parentElement?.className).toContain("right-[14.9%]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("max-w-[112px]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("left-[-2.18rem]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("top-[0.16rem]");
    expect(within(chatHero).getByText(/^data mining$/i)).toHaveClass("max-w-[112px]");
    ...
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("left-[2.9rem]");
    ...
    expect(chatHero.querySelector("span[class*='w-[11.9%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[11.7%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='top-[28.4%]'][class*='w-[11.7%]'][class*='rotate-[13deg]']")).not.toBeNull();
