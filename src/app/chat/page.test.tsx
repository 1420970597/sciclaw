...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("left-[-1.88rem]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("top-[0.46rem]");
    ...
    expect(within(chatHero).getByText(/^data mining$/i)).toHaveClass("top-[calc(100%-0.08rem)]");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("max-w-[126px]");
    ...
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("top-[0.36rem]");
    expect(within(chatHero).getByText(/^outcome present$/i)).not.toHaveClass("-translate-y-1/2");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("sm:left-[1.88rem]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("sm:left-[-1.94rem]");
    ...
    expect(chatHero.querySelector("span[class*='w-[9.6%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[9.9%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[9.42%]'][class*='top-[27.66%]'][class*='rotate-[10deg]']")).not.toBeNull();
    expect(chatHero).not.toHaveClass("xl:grid-cols-[minmax(0,1.08fr)_232px]");
    expect(chatAuthCard).toHaveClass("max-w-[220px]");
    expect(chatAuthCard).toHaveClass("lg:max-w-[492px]");
    expect(chatAuthCard).toHaveClass("xl:max-w-[508px]");
    expect(chatAuthCard).toHaveClass("rounded-[0.74rem]");
    expect(chatAuthCard).toHaveClass("border-[rgba(247,249,252,0.74)]");
    expect(chatAuthCard).toHaveClass("bg-[linear-gradient(180deg,rgba(255,255,255,0.958)_0%,rgba(249,244,237,0.38)_100%)]");
    expect(chatAuthCard).toHaveClass("shadow-[0_1px_2px_rgba(15,23,42,0.0018),0_1px_2px_rgba(241,180,135,0.00038)]");
    expect(chatAuthCard).toHaveClass("backdrop-blur-[0.05px]");
    expect(chatAuthCard).toHaveClass("mx-auto");
    expect(chatAuthCard.firstElementChild).toHaveClass("max-w-[252px]");
    expect(chatAuthCard.firstElementChild).toHaveClass("lg:max-w-[496px]");
    expect(chatAuthCard.firstElementChild).toHaveClass("xl:max-w-[512px]");
    expect(chatAuthCard.firstElementChild?.firstElementChild).toHaveClass("rounded-[0.78rem]");
    expect(chatAuthCard).not.toHaveClass("max-w-[224px]");
    expect(chatAuthCard).not.toHaveClass("lg:max-w-[508px]");
    expect(chatAuthCard).not.toHaveClass("xl:max-w-[524px]");
