...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(within(chatHero).getByTestId("feature-node-literature-analysis").parentElement?.className).toContain("left-[49%]");
    ...
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("left-[1.5rem]");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("top-[0.26rem]");
    expect(within(chatHero).getByText(/^outcome present$/i)).not.toHaveClass("-translate-y-1/2");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("sm:left-[1.58rem]");
    ...
    expect(chatHero.querySelector("span[class*='w-[10.1%]']")).not.toBeNull();
