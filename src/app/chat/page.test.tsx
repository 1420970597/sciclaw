...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(chatFeatureRotator).toHaveClass("lg:gap-[0.12rem]");
    expect(chatFeatureRotator).toHaveClass("xl:grid-cols-[minmax(0,342px)_minmax(0,1fr)]");
    expect(chatFeatureRotator).toHaveClass("xl:gap-[0.18rem]");
    ...
    expect(chatFeaturePreview).toHaveClass("lg:grid-cols-[minmax(0,1fr)_196px]");
    ...
    expect(chatFeaturePreview).toHaveClass("xl:grid-cols-[minmax(0,1fr)_196px]");
    ...
    expect(chatNetworkShellFrame).toHaveClass("lg:-mt-[2.24rem]");
    expect(chatNetworkShellFrame).toHaveClass("lg:mr-0");
    expect(chatNetworkShellFrame).toHaveClass("xl:-mt-[2.48rem]");
    expect(chatNetworkShellFrame).toHaveClass("xl:-mr-[0.04rem]");
    expect(within(chatHero).getByTestId("feature-node-literature-analysis").parentElement?.className).toContain("left-[46.45%]");
    expect(within(chatHero).getByTestId("feature-node-data-mining").parentElement?.className).toContain("left-[46.55%]");
    expect(within(chatHero).getByTestId("feature-node-outcome-present").parentElement?.className).toContain("right-[18.2%]");
    ...
    expect(chatHero.querySelector("span[class*='w-[12.05%]']")).not.toBeNull();
    ...
    expect(chatAuthCard).toHaveClass("max-w-[186px]");
    expect(chatAuthCard).toHaveClass("rounded-[1.08rem]");
    expect(chatAuthCard).toHaveClass("border-[rgba(244,247,250,0.97)]");
    expect(chatAuthCard).toHaveClass("bg-[linear-gradient(180deg,rgba(255,255,255,0.994)_0%,rgba(251,247,242,0.94)_100%)]");
    expect(chatAuthCard).toHaveClass("shadow-[0_2px_5px_rgba(15,23,42,0.018),0_1px_2px_rgba(241,180,135,0.009)]");
    expect(chatAuthCard).toHaveClass("backdrop-blur-[0.8px]");
