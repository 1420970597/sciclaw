...
describe("Chat route public landing clone", () => {
  it("renders the same marketing landing shell as the public live /chat route on fresh load", () => {
    ...
    expect(within(chatHero).getByTestId("feature-node-data-mining").parentElement?.className).toContain("left-[48.28%]");
    expect(within(chatHero).getByTestId("feature-node-outcome-present").parentElement?.className).toContain("right-[20.2%]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("max-w-[108px]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("left-[-2.02rem]");
    expect(within(chatHero).getByText(/^literature analysis$/i)).toHaveClass("top-[0.12rem]");
    expect(within(chatHero).getByText(/^data mining$/i)).toHaveClass("max-w-[108px]");
    expect(within(chatHero).getByText(/^data mining$/i)).toHaveClass("top-[calc(100%+0.04rem)]");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("max-w-[104px]");
    expect(within(chatHero).getByText(/^outcome present$/i)).not.toHaveClass("whitespace-nowrap");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("left-[2.84rem]");
    expect(within(chatHero).getByText(/^outcome present$/i)).toHaveClass("top-[0.1rem]");
    ...
    expect(chatHero.querySelector("span[class*='w-[11.25%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='w-[11.1%]']")).not.toBeNull();
    expect(chatHero.querySelector("span[class*='top-[28.62%]'][class*='w-[10.35%]'][class*='rotate-[12deg]']")).not.toBeNull();
    ...
    expect(chatAuthCard).toHaveClass("shadow-[0_4px_10px_rgba(15,23,42,0.018),0_2px_5px_rgba(241,180,135,0.006)]");
